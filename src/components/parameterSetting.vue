...
<template>
	<div class="parameter-setting">
		<a-form id="fourFrom" :form="form" layout="horizontal">
			<a-form-item :required="false" :label-col="{ span: 16 }" :wrapper-col="{ span: 8 }" :label="el.configDesc" v-for="(el, index) in fourData"
			 :key="index">
				<a-input style="color: rgba(50,50,50,1); font-weight: bold;" type="text" @pressEnter="carriageReturn(el.configName)" v-decorator="[
						`${el.configName}`,
						{
		...
		<a-modal width="1080px" class="set-form" okText="保存" centered :maskClosable="false" v-model="visible" @ok="handleOk">
			<div slot="title"><span style="font-size: 20px;font-weight: bold;">参数设置</span></div>
				...
				<a-form id="allFrom" :form="formData" layout="inline">
					<div :key="index" v-for="(all,index) in allData">
						...
						<div class="set-way-div">
							<a-form-item style="width: 31%;" v-for="(el,index) in all" :required="false" :label-col="{ span: 16 }"
							 :wrapper-col="{ span: 8 }" :label="el.configDesc" :key="index">
								<a-input type="text" style="width: 105px;" v-decorator="[
									`${el.configName}`,
									{
										initialValue: `${el.configValue}`,
		...
		methods: { //更多按钮触发
			...
			validatorList(rule, value, callback) { //全部数据正则表达式验证
				...
				if (rule.field == 'maximumOpeningOfShuntValve') { //分流阀最大开度//////第一个
					...
					} else if (value && Number(value) <= Number(form.getFieldValue('minimumValveOpening'))) {
 					callback(new Error('大于最小'));
 					return;
 				} else	{
 					callback();
 				}
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (value && Number(value) <= Number(form.getFieldValue('minimumOpeningOfRefillValve'))) {
 					callback(new Error('大于最小'));
 					return;
 				} else	{
 					callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (Number(value) && Number(form.getFieldValue('maximumOpeningOfShuntValve'))) {
 	    form.validateFields(['maximumOpeningOfShuntValve'], { force: true });
 	  }
 	  callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (Number(value) && Number(form.getFieldValue('maximumOpeningOfRefillValve'))) {
 	    form.validateFields(['maximumOpeningOfRefillValve'], { force: true });
 	  }
 	  callback();
					...
					} else if (Number(value) && Number(form.getFieldValue('cleanedCoalAshUpper'))) {
 	    form.validateFields(['cleanedCoalAshUpper'], { force: true });
 	  }
 	  callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('7-11小数3'));
					} else if (value && Number(value) <= Number(form.getFieldValue('cleanedCoalAshLower'))) {
 					callback(new Error('大于下限'));
 					return;
 				} else	{
 					callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('大于1小数2'));
					} else if (value && Number(value) <= Number(form.getFieldValue('lowerProductionDensity'))) {
 					callback(new Error('大于下限'));
 					return;
 				} else	{
 					callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('大于1小数2'));
					} else if (Number(value) && Number(form.getFieldValue('upperProductionDensity'))) {
 	    form.validateFields(['upperProductionDensity'], { force: true });
 	  }
 	  callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else  if (value && Number(value) <= Number(form.getFieldValue('superLowLevel'))) {
 					callback(new Error('大于超低'));
 					return;
 				} else	{
 					callback();
 				}
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (Number(value) && Number(form.getFieldValue('highLevel'))) {
 	    form.validateFields(['highLevel'], { force: true });
 	  }
 	  callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (Number(value) && Number(form.getFieldValue('superHighLevel'))) {
 	    form.validateFields(['superHighLevel'], { force: true });
 	  }
 	  callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (value && Number(value) <= Number(form.getFieldValue('lowLevel'))) {
 					callback(new Error('大于低'));
 					return;
 				} else	{
 					callback();
 				}
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (value && Number(value) <= Number(form.getFieldValue('peatLevelSetLow'))) {
 					callback(new Error('大于低'));
 					return;
 				} else	{
 					callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (Number(value) && Number(form.getFieldValue('peatLevelSetSuperHigh'))) {
 	    form.validateFields(['peatLevelSetSuperHigh'], { force: true });
 	  }
 	  callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (Number(value) && Number(form.getFieldValue('peatLevelSetHigh'))) {
 	    form.validateFields(['peatLevelSetHigh'], { force: true });
 	  }
 	  callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('0-100整数'));
					} else if (value && Number(value) <= Number(form.getFieldValue('peatLevelSetSuperLow'))) {
 					callback(new Error('大于超低'));
 					return;
 				} else	{
 					callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('大于1小数2'));
					} else  if (value && Number(value) <= Number(form.getFieldValue('ashLower'))) {
 					callback(new Error('大于下限'));
 					return;
 				} else	{
 					callback();
					...
					if (value && !value.match(mUserId)) {
						return callback(new Error('大于1小数2'));
					} else if (Number(value) && Number(form.getFieldValue('ashUpper'))) {
 	    form.validateFields(['ashUpper'], { force: true });
 	  }
 	  callback();
